...
// 散列函数（计算 cache 的 key）
function Hash (str) {
  for (var i = str.length, hash = 5381; i--;) { hash += (hash << 5) + str.charCodeAt(i) }
  return hash
};
// 动画
    ...
    this.getText = (whiteSpace = true) => {
      ...
      const DFS = (node) => {
        if (node.type == 'text') return text += node.text
          ...
          if (node.children) {
            for (var i = 0; i < node.children.length; i++) { DFS(node.children[i]) }
          }
        ...
        for (var j = node.data.nodes.length, item; item = node.data.nodes[--j];) {
          ...
          if (item.name == 'img') {
            if (item.attrs.src && item.attrs.i) {
              if (this.imgList.indexOf(item.attrs.src) == -1) { this.imgList[item.attrs.i] = item.attrs.src } else this.imgList[item.attrs.i] = Deduplication(item.attrs.src)
            }
            if (!observed) {
              observed = true;
              // 懒加载
