...
Component({
  ...
  methods: {
    ...
    playEvent (e) {
      if (this._top && (this._top.videoContexts || []).length > 1 && this._top.data.autopause) {
        for (var i = this._top.videoContexts.length; i--;) {
          if (this._top.videoContexts[i].id != e.currentTarget.id) { this._top.videoContexts[i].pause() }
        }
      }
    },
    // 图片预览事件
    ...
    videoError (e) {
      if (!this.loadSource(e.currentTarget) && this._top) { this.triggerError('video', e.currentTarget, e.detail.errMsg, undefined, this._top.getVideoContext(e.currentTarget.id)) }
    },
    audioError (e) {
      if (!this.loadSource(e.currentTarget)) { this.triggerError('audio', e.currentTarget, e.detail.errMsg) }
    },
    // 加载视频
    loadVideo (e) {
      this.setData({
